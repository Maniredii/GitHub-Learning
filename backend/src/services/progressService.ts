...
export class ProgressService {
  ...
  static async getUserProgress(userId: string): Promise<UserProgress | null> {
    const progress = await db<UserProgress>('user_progress').where({ user_id: userId }).first();
   ...
   */
  private static async getUnlockedQuests(userId: string, chapterId: string): Promise<string[]> {
