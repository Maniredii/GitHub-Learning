...
export class HintService {
  ...
  async upsertHintTracking(
    ...
    if (existing) {
      ...
      // Stringify shown_hint_indices if it's an array
      if (data.shown_hint_indices && Array.isArray(data.shown_hint_indices)) {
        updateData.shown_hint_indices = JSON.stringify(data.shown_hint_indices);
      }
      ...
      // Create new record
      await db('quest_hint_tracking').insert({
        user_id: userId,
        quest_id: questId,
