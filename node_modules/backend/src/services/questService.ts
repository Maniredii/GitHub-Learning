...
export class QuestService {
   ...
   */
  async getAllQuests(filters?: { chapterId?: string; unlocked?: boolean; userId?: string }): Promise<Quest[]> {
    let query = db('quests').select('*').orderBy([
      { column: 'chapter_id' },
      { column: 'order' },
    ]);
    ...
    if (filters?.unlocked !== undefined && filters?.userId) {
      ...
      return quests.filter((quest: Quest) => {
        ...
        const isChapterUnlocked = this.isChapterUnlocked(chapter, userProgress, completedQuests, chapters);
  ...
  async getQuestsWithLockStatus(userId: string): Promise<Array<Quest & { isLocked: boolean }>> {
    ...
    return quests.map((quest: Quest) => {
      const chapter = chapterMap.get(quest.chapter_id);
      const isLocked = chapter ? !this.isChapterUnlocked(chapter, userProgress, completedQuests, chapters) : true;
