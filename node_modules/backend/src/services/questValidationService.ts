...
export class QuestValidationService {
   ...
   */
  private validateCommitExists(parameters: Record<string, any>, state: RepositoryState): ValidationResult {
   ...
   */
  private validateBranchExists(parameters: Record<string, any>, state: RepositoryState): ValidationResult {
    ...
    if (branchName !== undefined) {
      ...
      if (!hasBranch) {
        return {
          ...
          details: { expectedBranch: branchName, availableBranches: state.branches.map(b => b.name) },
        };
      }
    }
   ...
   */
  private validateFileContent(parameters: Record<string, any>, state: RepositoryState): ValidationResult {
   ...
   */
  private validateMergeCompleted(parameters: Record<string, any>, state: RepositoryState): ValidationResult {
    ...
    if (noConflicts) {
      ...
      const hasConflicts = Object.values(state.workingDirectory).some(file => 
      ...
      if (hasConflicts) {
        return {
          success: false,
          feedback: 'There are still unresolved conflicts in your files. Please resolve them before completing the merge.',
        };
      }
    }
  ...
  private validateCustom(
    ...
    const { requiresManualCompletion, checkRepositoryInitialized, checkStagingArea, minimumFiles } = parameters;
   ...
   */
  private compareStates(actualState: RepositoryState, expectedState: RepositoryState): ValidationResult {
    // Compare commits
