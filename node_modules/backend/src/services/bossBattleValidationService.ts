...
export class BossBattleValidationService {
  ...
  private validateCorruptedTimeline(
    ...
    const currentBranch = repositoryState.branches.find(
      (b) => b.name === repositoryState.head
    );
  ...
  private validateConvergenceConflict(
    ...
    // Get current commit
    const currentCommit = repositoryState.commits.find(
      (c) => c.hash === currentBranch.commitHash
    );
    if (!currentCommit) {
      return {
        success: false,
    ...
    if (mustHaveMergeCommit) {
      const hasTwoParents =
        currentCommit.parents && currentCommit.parents.length === 2;
